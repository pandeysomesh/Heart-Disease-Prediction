...
model = pickle.load(open('module/rf_classifier.pkl', 'rb'))
scaler = pickle.load(open('module/scaler.pkl', 'rb'))
...
def predict_route():
    if request.method == 'POST':
        ...
        print(f"Received male: {male} (type: {type(male)})")  # Debugging line
        age = int(request.form['age'])
        currentSmoker = request.form['currentSmoker']
        cigsPerDay = float(request.form['cigsPerDay'])
